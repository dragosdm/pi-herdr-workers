...
export class LoopWidget {
  ...
  constructor(
    ...
    private monitorManager: MonitorManagerLike,
  ) {}
  ...
  private computeStatus(): string | undefined {
    ...
    if (monitors.length === 1 && monitor && monitor.startedAt !== undefined) {
      const activity = formatMonitorActivity({ ...monitor, startedAt: monitor.startedAt });
