...
```json
{
  ...
  "correlationId": "dispatch-research-1",
...
```json
{
  "runId": "f42fcf01-2d90-4e63-936f-ff1d97262999",
  "correlationId": "dispatch-research-1",
  "name": "agent-scout",
  "paneId": "%12",
  "cwd": "/workspace/project",
...
`runId` is generated by the provider after pure request validation and before worker side effects. It identifies one assignment across the spawn result, worker binding, inbox reports, and lifecycle journal. Optional `correlationId` is supplied by the caller and echoed in the result and lifecycle observations. It can hold a controller dispatch ID, but it is not required to be globally unique.

These identities are separate from `requestId`: retrying the same or a new RPC request does not reuse or deduplicate a run. A successful reply proves that the provider's creation path returned; assignment completion requires an explicit run-aware lifecycle report. See `docs/lifecycle-events.md`.
...
Spawn may leave external side effects when a request times out or is aborted. Timeout and abort settle only the caller's wait; they are not forwarded as provider cancellation and do not manufacture provider lifecycle outcomes. The provider can continue, emit a late reply after the caller removed its listener, and record lifecycle evidence for the generated run. The protocol does not roll back panes and does not make retries idempotent; reconcile ambiguous outcomes before retrying.
...
| `correlationId`, returned `runId` | 1-128 safe-ID characters |
