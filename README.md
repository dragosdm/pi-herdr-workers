...
- Cron: supported shorthand such as `5m` or `2h`, or a [local-time numeric five-field cron expression](docs/cron-semantics.md)
- Event: a Pi event source
- Hybrid: a cron schedule plus an event source, configured with `LoopCreate`
- Idle/dynamic: `/loop <goal>` then `LoopUpdate` with the wake's required `wakeId` and `continue` / `paused` / `completed`

Each ordinary dynamic dispatch saves a UUID with its fire count before emitting the wake. Copy the exact `Wake ID` from that message:

```text
LoopUpdate id="1" wakeId="<Wake ID from this wake>" status="continue" state="Checks complete; work remains" nextInterval="3m"
```
...
Cron day matching has been corrected: two restricted day fields use OR, so `0 0 1 * 1` runs on the first of each month and on Mondays. If either day token starts with `*`, including `*/n`, both day predicates must match. Existing expressions receive these semantics on restoration or re-arm and may next wake sooner. No past occurrences are replayed, and counters, IDs, expiry times, and paused status are unchanged. See [cron semantics](docs/cron-semantics.md) for the numeric grammar, DST behavior, bounded search, and expiry-only schedules.
...
- `MonitorList` reads one fresh `recent-unwrapped` text snapshot of up to 50 terminal rows per pane, including after reload. Herdr 0.8.0 may select blank bottom-screen rows, so the capture window is wider than the display limit. It shows at most the last five nonblank captured rows per pane, each limited to 100 JavaScript string code units. ANSI/VT escapes are removed, carriage returns become line breaks, and remaining control characters become spaces. Indentation and printable Unicode are retained; this is a display tail, not a byte-for-byte export.
- A successful snapshot with no displayable text shows `(no output captured)`. A read failure shows `(could not read pane)` without changing the observed process status or hiding other panes' output. Empty capture does not prove that a command produced no output; older text outside the bounded 50-row window is not fetched.
- Tails are terminal context and may include shell prompts or older output. They are not proof of launch, completion, or success. Output is not cached or persisted; reload reads the pane again without restarting the command.
...
- A paused checkpoint restores from the latest matching `herdr-loops.snapshot.v1` in all session entries, or from the authoritative shared file. Restoration does not fire a paused loop; the next wake after explicit `/loop` Resume uses that checkpoint. Supplementary `herdr-loops.update.v1` history is not replayed and cannot repair checkpoints lost before the [A09 fix](docs/implementation/INTEGRATION.md). Legacy imports preserve only the checkpoint actually saved.
- Dynamic acknowledgements consume their token in the same locked mutation as the checkpoint/status change. Reload retains an outstanding token exactly and never reopens an accepted one. A crash after saving a wake but before queueing its notification leaves an awaiting controller for inspection, not an automatic replay. This is at-most-one accepted update per recorded wake, not exactly-once model execution, message delivery, external effects or power-loss durability. After an auxiliary audit, shared-file mirror or trigger re-registration error, inspect the authoritative snapshot; the token may already be consumed. Do not select a new token and retry old work automatically. Normal reload/re-arm recovers saved scheduling state.
...
The [20 September 2026 audit](docs/audits/2026-09-20-live-extension.md) records the original defects and testing limits. All eleven findings now have positive regression tests and independent review. The [acceptance record](docs/implementation/INTEGRATION.md) covers 1,203 passing combined tests, the timezone matrix, required live checks, and remaining limitations.
