...
- `MonitorList` reads fresh `recent-unwrapped` text snapshots, including after reload. It shows at most five nonblank tail rows per pane, each limited to 100 JavaScript string code units. ANSI/VT escapes are removed, carriage returns become line breaks, and remaining control characters become spaces. Indentation and printable Unicode are retained; this is a display tail, not a byte-for-byte export.
- A successful snapshot with no displayable text shows `(no output captured)`. A read failure shows `(could not read pane)` without changing the observed process status or hiding other panes' output. Empty capture does not prove that a command produced no output.
