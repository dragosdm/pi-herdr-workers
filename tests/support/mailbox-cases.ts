...
export const mailboxCases = [
	...
	supported("real-watch-delivery", "ordinary", "Publish after a named readiness event on the same still-open watcher, polling disabled",
		"Within 3000 ms the final envelope's own fs.watch notification drives one queued custom message; the envelope remains",
		"Private local root, known peer, one receiver, named notifications; auxiliary watch closed before one marker write, primary marker event observed within 3000 ms; no watcher changes, setup-event scans, polling or manual drain during delivery",
		"Controlled host, real fs.watch after observed readiness; not immediate startup"),
	supported("startup-scan-without-notifications", "ordinary", "Final envelope exists before listener startup",
		"The startup scan queues one custom message and retains the envelope without any watch or poll callback",
		"Known peer, private root, captured watch and poll callbacks never invoked", "Controlled host, startup scan only"),
